...
import { ArrowUpRight, MapPin, Instagram, Target, Layout, PenTool } from "lucide-react";

const services = [
  {
    title: "Google Meu Negócio",
    description: "Se sua empresa não aparece no Google, você simplesmente não existe. Nós colocamos seu negócio no topo, com um perfil otimizado e profissional.",
    icon: MapPin,
    benefit: "Mais ligações e mensagens"
  },
  {
    title: "Instagram Profissional",
    description: "Seu Instagram precisa vender. Transformamos seu perfil em uma vitrine estratégica que atrai, engaja e converte seguidores em clientes.",
    icon: Instagram,
    benefit: "Mais autoridade e desejo"
  },
  {
    title: "Tráfego Pago",
    description: "Criamos campanhas estratégicas (Meta & Google Ads) que colocam sua empresa na frente das pessoas certas, todos os dias.",
    icon: Target,
    benefit: "Clientes chegando todo dia"
  },
  {
    title: "Sites & Landing Pages",
    description: "Criamos páginas modernas, rápidas e pensadas para conversão — feitas para transformar visitantes em oportunidades reais.",
    icon: Layout,
    benefit: "Conversão em escala"
  },
  {
    title: "Conteúdo & Criativos",
    description: "Design não é só estética — é percepção de valor. Criamos conteúdos que geram autoridade e fazem sua empresa parecer muito maior.",
    icon: PenTool,
    ...
    <section className="py-20 px-4 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto bg-brand-blue rounded-[40px] p-12 md:p-20 relative overflow-hidden border border-white/10">
        ...
        <div className="relative z-10 mb-16 flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
          <div>
            <span className="text-white/60 text-xs font-bold uppercase tracking-[0.4em] mb-4 block">O que a NexusWay faz</span>
            <h2 className="text-6xl md:text-8xl font-bold tracking-tight text-white">Serviços</h2>
          </div>
          ...
          {services.map((service, index) => (
            ...
            >
              ...
              <div className="pt-4 border-t border-white/5">
                <span className="text-[10px] font-bold text-brand-gold uppercase tracking-widest">{service.benefit}</span>
              </div>
            </motion.div>
          ))}
        </div>
